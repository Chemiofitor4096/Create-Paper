...
public class InteractEvent {
    ...
    private static void handleBucketPouring(PlayerInteractEvent.RightClickBlock event, ItemStack stack, Level level, BlockPos pos, Player player, InteractionHand hand, BlockState state) {
        ...
        if (state.is(Blocks.CAULDRON)) {
            if (com.chemiofitor.tponder.index.CPFluids.PULP.getBucket().isPresent() && stack.is(com.chemiofitor.tponder.index.CPFluids.PULP.getBucket().get())) {
                ...
                event.setCanceled(true);
            } else if (com.chemiofitor.tponder.index.CPFluids.FINE_PULP.getBucket().isPresent() && stack.is(com.chemiofitor.tponder.index.CPFluids.FINE_PULP.getBucket().get())) {
                var result = CauldronInteraction.emptyBucket(
                        level, pos, player, hand, stack,
                ...
                event.setCancellationResult(result);
                event.setCanceled(true);
            } else if (com.chemiofitor.tponder.index.CPFluids.XUAN_PAPER_PULP.getBucket().isPresent() && stack.is(com.chemiofitor.tponder.index.CPFluids.XUAN_PAPER_PULP.getBucket().get())) {
                var result = CauldronInteraction.emptyBucket(
                        level, pos, player, hand, stack,
    ...
    private static void handleBucketFilling(PlayerInteractEvent.RightClickBlock event, ItemStack stack, Level level, BlockPos pos, Player player, InteractionHand hand, BlockState state) {
        ...
        if (stack.is(Items.BUCKET)) {
            if (state.is(CPBlocks.PULP_CAULDRON.get())) {
                if (com.chemiofitor.tponder.index.CPFluids.PULP.getBucket().isPresent()) {
                    var result = CauldronInteraction.fillBucket(state, level, pos, player, hand, stack, new ItemStack(com.chemiofitor.tponder.index.CPFluids.PULP.getBucket().get()), (s) -> s.getValue(LayeredCauldronBlock.LEVEL) == 3, SoundEvents.BUCKET_FILL);
                    event.setCancellationResult(result);
                    event.setCanceled(true);
                }
            } else if (state.is(CPBlocks.FINE_PULP_CAULDRON.get())) {
                if (com.chemiofitor.tponder.index.CPFluids.FINE_PULP.getBucket().isPresent()) {
                    var result = CauldronInteraction.fillBucket(state, level, pos, player, hand, stack, new ItemStack(com.chemiofitor.tponder.index.CPFluids.FINE_PULP.getBucket().get()), (s) -> s.getValue(LayeredCauldronBlock.LEVEL) == 3, SoundEvents.BUCKET_FILL);
                    event.setCancellationResult(result);
                    event.setCanceled(true);
                }
            } else if (state.is(CPBlocks.XUAN_PAPER_PULP_CAULDRON.get())) {
                if (com.chemiofitor.tponder.index.CPFluids.XUAN_PAPER_PULP.getBucket().isPresent()) {
                    var result = CauldronInteraction.fillBucket(state, level, pos, player, hand, stack, new ItemStack(com.chemiofitor.tponder.index.CPFluids.XUAN_PAPER_PULP.getBucket().get()), (s) -> s.getValue(LayeredCauldronBlock.LEVEL) == 3, SoundEvents.BUCKET_FILL);
                    event.setCancellationResult(result);
                    event.setCanceled(true);
                }
            }
